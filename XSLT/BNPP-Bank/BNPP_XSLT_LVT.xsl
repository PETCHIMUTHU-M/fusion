...
<!--End of payment type information block-->
 ...
 <DbtrAgt>
 <FinInstnId>
 <!-- <xsl:if test="not(InstructionGrouping/BankAccount/SwiftCode='')"> -->
 <BIC>
  <xsl:value-of select="InstructionGrouping/BankAccount/SwiftCode" /> 
  </BIC>
  <!-- </xsl:if> -->
 <xsl:if test="(InstructionGrouping/BankAccount/SwiftCode='')">
...
not(SettlementPriority/Code=''))">
 ...
 <xsl:if test="(PayeeBankAccount/SwiftCode='')">
  ...
  <AdrLine>
  <xsl:value-of select="BankAddress/AddressLine1" /> 
  </AdrLine>
  </PstlAdr>
  </xsl:if>
  </FinInstnId>
 ...
 <Ustrd>
 <xsl:value-of select="PaymentReasonComments" />
 </Ustrd>
 <!-- <Strd>
 <Invcr>
 <Nm>
